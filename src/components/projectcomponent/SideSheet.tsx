...
const QuerySheet = () => {
  ...
  const onSubmitForm = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const token = await window.grecaptcha.execute(`6LfLTAcrAAAAAPO3Mi9us6VmTclabJOn2HQFAQZi`, { action: "submit" });
      ...
      <form onSubmit={onSubmitForm} className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {lookingForOptions.map((option) => (
              <label key={option.value} className={`flex items-center p-2 rounded-md border transition-all duration-200 text-sm 
                ...
                <input type="checkbox" value={option.value} checked={formData.lookingFor.includes(option.value)} onChange={handleCheckboxChange} className="sr-only" />
                <div className={`w-3.5 h-3.5 mr-1 flex items-center justify-center rounded border ${formData.lookingFor.includes(option.value) ? "bg-orange border-orange" : "border-gray-400"}`}>
        ...
        <div className="pt-3 space-y-2">
          <button type="submit" className={`w-full py-2.5 rounded-md text-sm font-medium transition-all duration-300 
            ...
            {loading ? <span className="flex items-center justify-center"><LoadingOutlined className="mr-2" /> Submitting...</span> : "Submit Query"}
          </button>

          <SheetClose asChild>
            <button type="button" className="w-full bg-gray-100 text-gray-700 py-2.5 rounded-md hover:bg-gray-200 transition-all duration-200 text-sm font-medium">
              Close
            </button>
          </SheetClose>
        </div>
