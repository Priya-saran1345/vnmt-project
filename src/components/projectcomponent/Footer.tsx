...
const Footer = ({ data }: any) => {
  ...
  return (
    <>
    <div className="hidden bg-[url('/images/footer-banner.svg')] -mt-20 max-w-[1246px] items-end pb-12 md:flex justify-start px-12 z-10 z-8 relative bg-no-repeat mx-auto h-[350px] gap-12 rounded-3xl text-white py-4">
      <div className='w-[45%]'>
        ...
        <p className='paragraph'>
          ...
          Contact us today to begin your journey toward success
        </p>
      </div>
      <div className='mb-9 ml-4'>
        <button className='_button1 popup-effect'>
          Book Consultant Now
        </button>
      </div>
    </div>
      ...
      <div className="-mt-20 pt-20 text-white text-sm overflow-hidden relative bg_footer_img z-[1]">
        ...
        <div className="bg-black md:bg-transparent flex items-center justify-center flex-col w-full mx-auto container">
          <div className='w-full'>
            ...
            {/* INDUSTRIES THAT WE SERVE - Mobile responsive changes */}
            <div className="flex flex-col gap-4 z-10 px-4">
              ...
              <div className="flex flex-wrap gap-2 text-[14px] relative z-10">
                {industries.map((industry, index) => (
                  <span key={index} className="inline-flex">
                    {index !== 0 && <span className="mx-1">|</span>}
                    <span className="hover:text-orange text-white cursor-pointer whitespace-nowrap">{industry}</span>
                  </span>
                ))}
              </div>
            </div>
            ...
            <div className="w-full px-4">
              {/* Footer Logos - Mobile responsive changes */}
              <div ref={sectionRef} className="flex flex-col md:flex-row justify-between items-center">
                <div className="z-10 w-full md:flex-1 overflow-hidden">
                  <motion.div
                    initial={{ opacity: 0 }}
                  ...
                  >
                    <div className="infinite-scroll gap-4 md:gap-8">
                ...
                {/* Quick Connect - Mobile responsive changes */}
                <div className="flex gap-8 items-end mt-4 md:mt-0">
                  <div className="flex fixed bottom-8 right-[3%] flex-row-reverse z-[1000] gap-6 items-center justify-center">
                    <div className="relative group">
                      ...
                      <div className="absolute bottom-12 text-nowrap opacity-0 -left-4 group-hover:opacity-100 transition-opacity duration-300 bg-orange text-white text-xs rounded py-1 px-2">
                        Scroll to Top
                      </div>
                    </div>
                    <div className="bg-[#fff] p-3 md:p-4 rounded-xl shadow-lg animate-zoom">
                      <p className="text-blue text-center font-semibold mb-2 text-base md:text-lg">Quick Connect</p>
                      <a className="flex gap-2 items-center cursor-pointer group z-50" href={`https://wa.me/${data?.quick_whatsapp}`} target="_blank" rel="noopener noreferrer">
                        <IoLogoWhatsapp className="text-green-500 text-[20px] md:text-[23px] cursor-pointer" />
                        <p className="text-black group-hover:text-orange cursor-pointer smooth1 text-sm md:text-base">Whatsapp Now</p>
                      </a>
                      <a className="flex gap-2 items-center cursor-pointer group z-50 mt-2" href={`tel:${data?.quick_phone}`} target="_blank" rel="noopener noreferrer">
                        <FiPhoneCall className="text-black text-[14px] md:text-[16px] cursor-pointer" />
                        <p className="text-black group-hover:text-orange cursor-pointer smooth1 text-sm md:text-base">{data?.quick_phone}</p>
                      </a>
                    </div>
                  </div>
                </div>
